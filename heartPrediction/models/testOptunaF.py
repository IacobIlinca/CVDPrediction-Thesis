...
best_params = {'n_estimators': 48, 'max_depth': 5, 'min_samples_split': 8, 'min_samples_leaf': 4, 'max_features': 'log2'}
...
y_train_pred = pipeline.predict(X_trainF)
y_test_pred = pipeline.predict(X_testF)

# Compute F1 scores for both training and test sets
f1_train = f1_score(y_trainF, y_train_pred, average='weighted')
f1_test = f1_score(y_testF, y_test_pred, average='weighted')
...
plt.savefig('../results/modelResults/optuna/best/confusion_matrix' + "F" + '.png')
...
review_data = {'Classififer': ['Extra Tree with Optuna'], 'DataSet': ["F"], 'Accuracy': [accuracy], 'F1 Score': [f1],
               'Recall': [recall],
               'Precision': [precision]}
...
plt.savefig('../results/modelResults/optuna/best/metrics_table' + "F" + '.png')
